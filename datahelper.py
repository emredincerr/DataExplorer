...
def summarize_csv(data_file):
    ...
    data_summary["inital_data_sample"] = df.head()
    data_summary["column_descriptions"] = pandas_agent.run("Verideki sütunları içeren bir tablo yap. Tabloda sütunların adları ve yanlarında kısaca içerdikleri bilgilere dair Türkçe bir açıklama yer alsın. Bunu bir tablo olarak ver.")
...
def analyze_trend(data_file, veriable_of_interest):
    
    df = pd.read_csv(data_file, low_memory=False)
    
    pandas_agent = create_pandas_dataframe_agent(selected_llm, df, verbose=True, agent_executor_kwargs={"handle_parsing_errors": "True"})
    
    trend_response = pandas_agent.run(f"Veri kümesi içindeki şu değişkenin trendini kısaca yorumla: {veriable_of_interest} Yorumlamayı reddetme. Verideki satırlar geçmişten günümüze tarih bazlı olduğu için, verideki satırlara bakarak yorumda bulunabilirsin. Yanıtını Türkçe olarak ver.")
