...
loaded_file = st.sidebar.file_uploader(label="Yüklemek istediğini csv dosyanızı seçiniz", type="csv")

loaded_data_btn = st.sidebar.button(label="Yükle", on_click=activate_dataload, use_container_width=True)
...
with col_interaction:
    ...
    @st.cache_data
    def explore_veriable(data_file, veriable_of_interest):
        
        data_file.seek(0)
        dataframe = datahelper.get_dataframe(data_file=data_file)
        st.bar_chart(data=dataframe, y=[veriable_of_interest])
        st.divider()
        data_file.seek(0)
        trend_response = datahelper.analyze_trend(data_file=loaded_file, veriable_of_interest=veriable_of_interest)
        st.success(trend_response)
        return 
        
    if variable_of_interest or examine_btn:
        explore_veriable(data_file=loaded_file, veriable_of_interest=variable_of_interest)
        
    free_question = st.text_input(label="Veri Kümesi ile İlgili Ne Bilmek İstersiniz?")
    ...
    def answer_question(data_file, question):
        data_file.seek(0)
        AI_Response = datahelper.ask_question(data_file=datahelper, question=question)
